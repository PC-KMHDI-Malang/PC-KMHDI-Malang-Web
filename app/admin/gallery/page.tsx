...
export default async function GalleryPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-10">
        <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 mb-2">Manajemen Galeri</h1>
        <p className="text-slate-500 text-lg">Kelola foto-foto dokumentasi kegiatan KMHDI Malang.</p>
      </div>
      ...
      <div className="bg-white/70 backdrop-blur-xl p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 mb-10">
        ...
        <form action={addGallery} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1.5">URL Foto</label>
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1.5">Deskripsi Singkat (Opsional)</label>
            <textarea name="description" className="w-full bg-slate-50 border border-slate-200 focus:border-red-500 focus:ring-4 focus:ring-red-500/10 rounded-xl p-3 outline-none transition-all" rows={3} placeholder="Tuliskan deskripsi..."></textarea>
          </div>
          <div className="pt-2">
      ...
      <div className="bg-white/70 backdrop-blur-xl p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-100">
          ...
          <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-sm font-semibold">{galleries?.length || 0} Foto</span>
        </div>
        {error && <p className="text-red-500 mb-4 font-medium">Gagal mengambil data.</p>}
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {galleries?.map((item) => (
            <div key={item.id} className="group relative rounded-2xl overflow-hidden bg-slate-50 border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
              ...
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <SubmitWithConfirm
                  ...
                  buttonElement={
                    <div className="w-10 h-10 flex items-center justify-center bg-red-600/90 text-white rounded-full hover:bg-red-700 hover:scale-110 transition-transform shadow-xl backdrop-blur-sm" title="Hapus Foto">
