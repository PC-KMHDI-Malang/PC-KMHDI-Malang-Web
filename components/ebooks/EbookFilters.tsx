    ...
    <div className="rounded-3xl bg-white dark:bg-[#111111] border border-slate-100 dark:border-white/5 p-6">
      ...
      <div>
        ...
        <div className="space-y-2.5">
          {genres.map((genre) => (
            <label key={genre} className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-300 cursor-pointer">
              <input
                type="checkbox"
          ...
          {genres.length === 0 && <p className="text-xs text-slate-400">Belum ada kategori.</p>}
        </div>
      </div>
      ...
      {(activeGenres.length > 0 || sortFilter !== "newest" || query) && (
        <button
          ...
          className="mt-6 w-full text-center text-xs font-bold text-red-600 dark:text-rose-500 hover:underline"
        >
          Reset Filter
        </button>
      )}
