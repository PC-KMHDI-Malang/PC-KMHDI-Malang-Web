...
interface AddNewsModalProps {
  ...
  usedBytes: number;
  articleImagesUsedBytes: number;
  action: (formData: FormData) => Promise<{ error?: string; success?: boolean; message?: string }>;
}

export function AddNewsModal({ action, usedBytes, articleImagesUsedBytes }: AddNewsModalProps) {
      ...
      {isRendered &&
        createPortal(
          <div className="fixed inset-y-0 right-0 left-0 md:left-64 z-[100] flex items-center justify-center p-4 text-left">
            ...
            <div
              className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white dark:bg-[#111114] rounded-3xl shadow-2xl p-8 transform transition-all duration-300 border border-slate-200 dark:border-white/10 ${
              className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white dark:bg-[#111111] rounded-3xl shadow-2xl p-8 transform transition-all duration-300 border border-slate-200 dark:border-white/5 ${
              ...
              <form action={handleFormAction} className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    name="title"
                    required
                    className="w-full bg-slate-50 dark:bg-[#111114] dark:text-white border border-slate-200 dark:border-white/5 focus:border-red-500 dark:focus:border-rose-500 focus:ring-4 focus:ring-red-500/10 dark:focus:ring-rose-500/20 rounded-xl p-3 outline-none transition-all text-lg font-semibold"
                ...
                <div>
                  ...
                  <RichTextEditor name="content" usedBytes={articleImagesUsedBytes} />
                ...
                <div>
                  ...
                  <ImagePicker bucket="news-covers" />
                </div>
