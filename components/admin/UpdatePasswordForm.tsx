...
export function UpdatePasswordForm() {
  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-6">
      ...
      {state?.success && (
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-600 text-sm font-bold flex items-center gap-3">
      ...
      <div>
        ...
        <input 
          ...
          required 
          className="w-full bg-slate-50 border border-slate-200 focus:border-slate-800 focus:ring-4 focus:ring-slate-800/10 rounded-xl p-3 outline-none transition-all font-medium" 
        />
      </div>
      <div>
        <label className="block text-sm font-bold text-slate-700 mb-1.5">Password Baru (Min. 6 Karakter)</label>
        <input 
          type="password" 
          name="newPassword" 
          required 
          minLength={6}
          className="w-full bg-slate-50 border border-slate-200 focus:border-red-500 focus:ring-4 focus:ring-red-500/10 rounded-xl p-3 outline-none transition-all font-medium" 
        />
      </div>
      <div className="pt-4 flex justify-end">
        <button 
          type="submit" 
          disabled={isPending}
          className={`bg-slate-900 text-white font-bold px-8 py-3 rounded-xl transition-all duration-300 shadow-sm flex items-center gap-2 ${
            isPending ? 'opacity-70 cursor-not-allowed' : 'hover:bg-black hover:shadow-lg hover:shadow-slate-900/30'
          }`}
        >
